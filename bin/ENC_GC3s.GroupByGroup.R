...
argv <- commandArgs(T)
if (length(argv) != 3){
    stop("Rscript ENC_GC3s.GroupByGroup.R [allGene.ENC_GC3s.txt] [The order of groupColumns] [prefix of output]")
}
...
plots <- list()
for (i in groupOrder){
  temp<-table[table$Groups == i,]
  rect_values <-rect_point(temp)
  
  ## orginal
  p1 <- ggplot(temp,aes(GC3s,ENC))+geom_point(aes(color=gene),size=1)+
    scale_shape_manual("Lineage",values=c(0,1,2))+geom_segment(data=rect_values,aes(x=x1,xend=x2,y=y1,yend=y2),color="grey20",linetype="dashed",size=0.3)+
   ...
   ## large
   p2 <- ggplot(temp,aes(GC3s,ENC))+geom_point(aes(color=gene),size=0.6)+geom_line(data=enc_gc3s,color="#7f7f7f")+
   ...
   legend_b <- get_legend(p1+theme(legend.position="bottom")+guides(colour = guide_legend(override.aes = list(size=2))))
   
   plots[[i]] <- p1 + annotation_custom(ggplotGrob(p2), xmin=0.08, xmax=0.92, ymin=1.8, ymax=35.2)
}
dev.off()

pdf(file=paste(argv[3],".ENC-GC3s.GroupByGroup.pdf",sep=''),width=4*length(unique(table$Groups)),height=5.5)
prow <- plot_grid(plotlist=plots,ncol=3,align=c("h"),labels=LETTERS[1:3],hjust=-1)
plot_grid( prow,legend_b, ncol = 1, rel_heights = c(20,2))
dev.off()
