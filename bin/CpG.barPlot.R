...
argv <- commandArgs(T)
if (length(argv) != 3){
    stop("Rscript CpG.barPlot.R [allGene.dinuc_frequency.txt] [The order of groupColumns][prefix of output]")
}
...
temp_mean<-dat %>% group_by(gene,Groups) %>% dplyr::summarise(mean=mean(CG))
temp_std<-dat %>% group_by(Groups,gene) %>% dplyr::summarise(std=sd(CG))
temp_mean['std'] <- temp_std['std']

###### order gene ####
gene_enc_mean<-dat %>% group_by(gene) %>% dplyr::summarise(mean=mean(CG))
order_gene<-gene_enc_mean[order(gene_enc_mean$mean,decreasing = T),]$gene

temp <- dat %>% group_by(gene,Groups) %>% dplyr::summarise(mean=mean(CG))
